...
while ((match = idRegex.exec(text)) !== null) {
  ...
  candidates.push(id);
  // numeric prefix handling: e.g., 001-aurora -> 1-aurora, 01-aurora
  ...
  if (!found) {
    // Try direct match with any file containing slug
  ...
  // Insert previewImageUrl before the closing brace of the object, trying to place near previewSrc if possible
  // Prefer to insert after previewSrc
  const previewSrcIdx = objText.search(/previewSrc\s*:/);
  let insertPosInObj = -1;
  if (previewSrcIdx !== -1) {
    // find end of the line after previewSrc
    const after = objText.indexOf('\n', previewSrcIdx);
    insertPosInObj = after !== -1 ? after + 1 : objText.length - 1;
  } else {
    // find end before closing brace
